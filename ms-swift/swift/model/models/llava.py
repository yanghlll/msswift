...
class LlavaOnevision2Loader(ModelLoader):
    def __init__(self, *args, **kwargs):
        ...
        if self.auto_model_cls is None:
            ...
            self.auto_model_cls = AutoModelForImageTextToText
    
    def get_model(self, model_dir: str, *args, **kwargs) -> PreTrainedModel:
        model_cls = get_class_from_dynamic_module(
            'modeling_llava_onevision2.LlavaOnevision2ForConditionalGeneration', model_dir)
...
register_model(
    ModelMeta(
        ...
        requires=['transformers>=5.7.0', 'decord'],
        tags=['vision', 'video'],
    ))
